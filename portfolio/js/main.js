...
// === Logo Video Theme Switch ===
function setLogoVideoByTheme() {
  var theme = document.documentElement.getAttribute('data-bs-theme');
  var video = document.getElementById('logoVideo');
  if (!video) return;
  var source = video.querySelector('source');
  if (!source) return;
  var newSrc = (theme === 'dark') ? 'images/darklogo.mp4' : 'images/lightlogo.mp4';
...
// Listen for theme changes (using MutationObserver for data-bs-theme changes)
...
// Also listen for the theme toggle switch (if it directly changes the theme)
var themeToggle = document.getElementById('themeToggle');
if (themeToggle) {
  themeToggle.addEventListener('change', function() {
    setTimeout(setLogoVideoByTheme, 10); // slight delay in case theme is set asynchronously
  });
}
